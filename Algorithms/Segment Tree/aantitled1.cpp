...
bool horizontal(int ux,int uy,int vx,int vy, char ch)
{
    ...
    for(int i=0; i<cnt; i++)
    {
        ...
        if(ch=='N'){
            if(uy>ay&&uy<=by){
                if(vx==ax) return false;
            }
        }
        else{
            if(uy>ay&&uy<=by){
                if(ux==ax) return false;
            }
        }
    }
...
int bfs()
{
    ...
    while(!q.empty())
    {
        ...
        for(int k=0; k<4; k++)
        {
            ...
            if(VALID(tx,ty)==false) continue;

            if(k<2){    //horizontal
                if(horizontal(ux,uy,tx,ty,dir[k])==false) continue;
            }
            else{   //vertical
                if(vertical(ux,uy,tx,ty,dir[k])==false) continue;
            }
