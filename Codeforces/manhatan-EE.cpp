...
ll fun(int pos,int choto,int boro,int suru,int mask)
{
    if(pos==sz){
    ...
    if(dp[pos][choto][boro][suru][mask]!=-1) return dp[pos][choto][boro][suru][mask];
    ...
    for(int i=lo; i<=hi; i++){
        ...
        ret += fun(pos+1,choto|(i<hi),boro|(i>lo),suru|(i>0),next_mask);
    }
    return dp[pos][choto][boro][suru][mask] = ret;
}
int main()
{
    ll tt;
