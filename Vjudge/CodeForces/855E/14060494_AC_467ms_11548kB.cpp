...
ll fun(int b,int p,int m,int f)
{
    if(p>SIZE[b]){
        //cout << ss << endl;
        //printf("f=%d m=%d b=%d\n\n",f,m,b);
...
//        string hh = ss;
//        hh += i+'0';
        if(i==0 && f==0) ret += fun(b,p+1,m,f);
        else ret += fun(b,p+1,m^(1<<i),f|(i>0));
    }
    //cout << ss << endl;
   // printf("b=%d p=%d m=%d f=%d dp=%lld\n\n",b,p,m,f,ret);
    return dp[b][p][m][f] = ret;
}
ll go(int b,int p,int m,int f,int c)
{
    if(p>SIZE[b]){
        if(f && m==0)return 1LL;
        else return 0LL;
...
//        hh += i+'0';
        if(i==0 && f==0){
            ret += go(b,p+1,m,f,c|(i<hi));
        }
        else{
           // printf("***i=%d b=%d p=%d m=%d f=%d c=%d\n\n",i,b,p,m,f,c);
            ret += go(b,p+1,m^(1<<i),f|(i>0),c|(i<hi));
        }
    }
