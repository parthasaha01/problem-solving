...
data query(int nd,int b,int e,int x,int y){
    ...
    if(y<=m){
        ...
        f = query(rg,m+1,e,x,y);
    }
    else{
        data f1 = query(lf,b,m,x,m);
        data f2 = query(rg,m+1,e,m+1,y);
        //printf("f1 f1.p=%lld f1.s=%lld f1.i=%lld f1.sum=%lld\n",f1.prefix, f1.suffix, f1.infixx, f1.sum);
        //printf("f2 f2.p=%lld f2.s=%lld f2.i=%lld f2.sum=%lld\n",f2.prefix, f2.suffix, f2.infixx, f2.sum);

        f  = maxData(f1,f2);
        //printf("f f.p=%lld f.s=%lld f.i=%lld f.sum=%lld\n",f.prefix, f.suffix, f.infixx, f.sum);
    }
    return f;
}
