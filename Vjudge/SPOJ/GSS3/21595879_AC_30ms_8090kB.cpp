...
Given a sequence A of N(N<=50000) integers between -10000 and 10000.
On this sequence you have to apply M (M <= 50000) operations:
Modify the i-th element in the sequence or
...
void update(int nd,int b,int e,int x,int v){
    if(b==e && b==x){ N.prefix = N.suffix = N.bestSum = N.sum = a[b] = v; return; }
    if(x<=m)update(lf,b,m,x,v);
    else update(rg,m+1,e,x,v);
    N = maxData(L,R);
}
data query(int nd,int b,int e,int x,int y){
    if(b>y || e<x){ data INF; INF.prefix=INF.suffix=INF.bestSum=INF.sum=-1000000000; return INF; }
    if(b>=x && e<=y) return tree[nd];
    return maxData(query(lf,b,m,x,y), query(rg,m+1,e,x,y));
}
int main(){
    int n; scanf("%d",&n);
