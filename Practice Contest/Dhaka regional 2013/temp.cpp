...
const int Bigintlen=2200;
const int Blen=8;
const int64 base=100000000;
struct Bigint
{
	int len;
	int64 data[Bigintlen];
	...
	Bigint operator %(const Bigint &b)
	{
		if(b.len<=1 && b[1]==0){ printf("error! 0 mod!"); }
		int i,l1=(len-1)*Blen,l2=(b.len-1)*Blen;
		int64 x=data[len],y=b[b.len];
		while(x)x/=10,l1++;
		while(y)y/=10,l2++;
	...
	Bigint operator /=(const Bigint &b){return *this=(*this/b);}
	Bigint operator %=(const Bigint &b){return *this=(*this%b);}
	Bigint operator *=(int64 x) {return( *this=(*this *x));}
	Bigint operator +=(int64 x) {return( *this=(*this +x));}
	Bigint operator -=(int64 x) {return( *this=(*this -x));}
	Bigint operator /=(int64 x) {return( *this=(*this /x));}
	void read()
	{
		char c[Bigintlen*Blen+10];
		scanf("%s",c+1);
		int l=strlen(c+1);
		(*this).clear();
		int64 x;
	...
	void write()
	{
		...
		for(int i=len-1;i>=1;--i)
            printf("%0*I64d",Blen,data[i]);
	}
}p,q,pp,qq;

Bigint gcd(const Bigint &A,const Bigint &B)
{
	Bigint a=A,b=B,res=1;
