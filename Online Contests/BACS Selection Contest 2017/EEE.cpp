...
Matrix matpow(Matrix base, int p, int m){
    Matrix ans;
    f(i,0,l) f(j,0,l) ans.mat[i][j]= (i==j);
    while(p){
...
int main(){
	ft{
		...
		cases;space;
		if(l==1) {printf("%lld\n", xp(2, n, mo)); continue;}
		temp.mat[0][0]=temp.mat[0][l-1]=1;
		f(i,1,l) temp.mat[i][i-1]=1;
		//printmat(temp);
		temp=matpow(temp, n, mo);
		//printmat(temp);
		printf("%ld\n",temp.mat[0][0]);
